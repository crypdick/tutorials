...
PyTorch’s FSDP2 enables model sharding across nodes, allowing
distributed training of large models with a significantly smaller memory
footprint compared to standard Distributed Data Parallel (DDP). For a
more detailed overview of FSDP2, see `PyTorch’s official
documentation <https://docs.pytorch.org/tutorials/intermediate/FSDP_tutorial.html#getting-started-with-fully-sharded-data-parallel-fsdp2>`__.

This tutorial provides a comprehensive, step-by-step guide on
integrating PyTorch FSDP2 with Ray Train. Specifically, this guide
covers the following:

- A hands-on example of training an image classification model
- Configuring FSDP2 to mitigate out-of-memory (OOM) errors using mixed
  precision, CPU offloading, sharding granularity, and more
- Model checkpoint saving and loading with PyTorch Distributed
  Checkpoint (DCP)
...
# ~~~~~~~~~~~~~~~~
#
# The following function initializes a Vision Transformer (ViT) model
# configured for the FashionMNIST dataset:
...
# -------------------------------
#
# Below is the main training function that orchestrates the FSDP2 training
# process. The following sections implement each of the helper functions
# used in this training loop. First, make the necessary imports for the
# training function:
...
# ----------------------------
#
# PyTorch’s ``fully_shard`` enables sharding at various granularities. At
# the most granular level, you can shard every layer to minimize peak
# memory utilization, but this also increases communication costs between
# Ray Train workers. Experiment with different sharding granularities to
# find the optimal balance for your use case. This example only shards the
# encoder blocks—the largest layers in the Vision Transformer.
#
# Beyond sharding granularity, FSDP2 offers several configuration options
# to optimize performance and mitigate OOM errors:
#
# Device mesh configuration
# ~~~~~~~~~~~~~~~~~~~~~~~~~
#
# ``init_device_mesh`` configures a ``DeviceMesh`` that describes the
# training run’s device topology. This example uses a simple 1D mesh for
# data parallelism, but ``DeviceMesh`` also supports multi-dimensional
# parallelism approaches including tensor parallelism and pipeline
# parallelism. In many cases, integrating several types of parallelism can
# further help to improve training performance.
#
# For more information about advanced multi-dimensional parallelism
# configurations, see the `PyTorch device mesh
# documentation <https://docs.pytorch.org/tutorials/recipes/distributed_device_mesh.html>`__.
#
# CPU offloading
# ~~~~~~~~~~~~~~
#
# CPU offloading reduces GPU memory footprint by storing model components
# in the CPU. However, this comes with the trade-off of increased data
# transfer overhead between CPU and GPU during computation.
#
# **CPU offloading does the following:** - Stores sharded parameters,
# gradients, and optimizer states on CPU - Copies sharded parameters to
# GPU during forward/backward computation and frees them after use -
# Copies computed gradients to the CPU where PyTorch computes the
# optimizer step
#
# **When to use CPU offloading:** - When GPU memory is constrained - For
...
#
# It can be seen that CPU offloading significantly reduces the amount of
# GPU memory occupied by model parameters.
#
# Learn more about CPU offloading in the `PyTorch
# documentation <https://docs.pytorch.org/docs/stable/distributed.fsdp.fully_shard.html#torch.distributed.fsdp.CPUOffloadPolicy>`__.
#
# ``reshard_after_forward`` flag
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# ``fully_shard`` has a ``reshard_after_forward`` flag that enables
# all-gathered model weights to be freed immediately after the forward
# pass. This reduces peak GPU memory usage but increases the communication
# overhead between workers during the backward pass as parameters need to
...
# ~~~~~~~~~~~~~~~
#
# Enabling mixed precision accelerates training and reduces GPU memory
# usage with minimal accuracy impact.
#
...
# is halved.
#
# Learn more about mixed precision configuration on the `PyTorch
# documentation <https://docs.pytorch.org/docs/stable/distributed.fsdp.fully_shard.html#torch.distributed.fsdp.MixedPrecisionPolicy>`__.
#
...
# ----------------------------
#
# This section sets up distributed checkpointing, loads a distributed
# model from a checkpoint, saves distributed model checkpoints, and saves
# a model for inference.
#
# Distributed checkpoint wrapper setup
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# This section creates a checkpointing wrapper using PyTorch’s
# ``Stateful`` API to simplify distributed checkpoint management. From the
# PyTorch docs, this basic wrapper handles the complexities of saving and
# loading FSDP2 model states across multiple workers.
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# Load distributed checkpoints using ``dcp.load``, which automatically
# handles resharding when the number of workers changes between training
# runs. This flexibility allows you to resume training with different
...
# ~~~~~~~~~~~~~~~~~~~~~~
#
# The following function handles periodic checkpoint saving during
# training, combining metrics reporting with distributed checkpoint
# storage:
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# After training, it is often useful to consolidate sharded checkpoints
# into a single file for convenient sharing or inference. Unlike regular
# distributed checkpointing, this process produces a large artifact
# compatible with torch.load. To do so, the ``get_model_state_dict``
# function all-gathers parameter shards to rank 0, reconstructs the full
# state dict, and then saves the consolidated checkpoint to cluster
# storage.
#
# Note that a key limitation of this approach is that the entire model
# must be materialized in memory on rank 0. For large models, this can
# exceed the available CPU RAM and result in out-of-memory errors. In such
# cases, it is advised to keep the model in its sharded format and rely on
# distributed model loading for inference.
...
# --------------------------------------
#
# This section configures and launches the distributed training job using
...
# --------------------
#
# GPU memory profiling is a useful tool for monitoring and analyzing
# memory usage during model training. It helps identify bottlenecks,
# optimize resource allocation, and prevent OOM errors. PyTorch’s GPU
...
# ---------------------------------------
#
# After training completes, you can load the saved model for inference on
# new data. Ray Train loads the model in its unsharded form, ready for
...
# - Gained insight on configuring FSDP2 to balance training performance
#   and memory usage
